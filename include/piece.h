...
namespace MiniDoc {
  ...
  class PieceTable {
    ...
    class Iterator {
      ...
      inline string_t string() const { return string_t(m_string, m_length); }
    ...
    string_t line_string(size_t line) const {
      string_t string;
    ...
    // caching this would be wasteful
    void line_string(size_t line, string_t & out) const {
      range_string(line_start(line), line_end(line), out);
    }
    
    // caching this would be wasteful
    string_t range_string(offset_t start, offset_t end) const {
      string_t string;
    ...
    // caching this would be wasteful
    void range_string(offset_t start, offset_t end, string_t & out) const {
      out.resize(0);
      if (end - start == 0) {
        return;
      }
    ...
    // caching this would be wasteful
    string_t line_string_func(size_t line, std::function<void(std::string & out, const char_t *string, size_t length)> func) const {
      string_t string;
    ...
    // caching this would be wasteful
    void line_string_func(size_t line, string_t & out, std::function<void(std::string & out, const char_t *string, size_t length)> func) const {
      range_string_func(line_start(line), line_end(line), out, func);
    }
    
    // caching this would be wasteful
    string_t range_string_func(offset_t start, offset_t end, std::function<void(std::string & out, const char_t *string, size_t length)> func) const {
      string_t string;
    ...
    // caching this would be wasteful
    void range_string_func(offset_t start, offset_t end, string_t & out, std::function<void(std::string & out, const char_t *string, size_t length)> func) const {
      out.resize(0);
      if (end - start == 0) {
        return;
      }
    ...
    void dump(bool print_line = false, bool print_node_string = false) const {
      if (print_line) {
        for (int i = 0; i < lines_cached(this); ++i) {
